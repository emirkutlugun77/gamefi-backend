...
export class NftAdminController {
  ...
  @ApiOperation({
    ...
    description:
      'Retrieves all NFT collections directly from the Solana blockchain (blockchain is source of truth)',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      'Removes collection from database sync. NOTE: This does NOT delete from blockchain (blockchain is immutable). Only removes from local database.',
  })
  @ApiParam({
    name: 'name',
    ...
    schema: {
      ...
      properties: {
        ...
        message: {
          type: 'string',
          example:
            'Collection removed from database (still exists on blockchain)',
        },
      },
    },
  })
  ...
  @ApiOperation({
    ...
    description:
      'Retrieves all NFT types for a specific collection, or all types if no collection specified',
  })
  @ApiQuery({
    name: 'collection',
    description:
      'Collection name (optional - if not provided, returns all types)',
    example: 'VYBE_BUILDINGS_COLLECTION',
    required: false,
  })
  @ApiResponse({
  ...
  @ApiOperation({
    ...
    description:
      'Removes NFT type from database sync. NOTE: This does NOT delete from blockchain (blockchain is immutable). Only removes from local database.',
  })
  @ApiParam({
    name: 'id',
    ...
    schema: {
      ...
      properties: {
        ...
        message: {
          type: 'string',
          example:
            'NFT type removed from database (still exists on blockchain)',
        },
      },
    },
  })
  ...
  @ApiOperation({
    ...
    description:
      'Create or update store tab configuration (building, troops, others)',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Fetches all collections from the Solana program and syncs them to the database',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      'Fetches all NFT types from the Solana program and syncs them to the database',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      'Syncs both collections and NFT types from the Solana program to the database',
  })
  @ApiResponse({
    status: 200,
  ...
  async syncAll() {
    try {
      ...
      const collectionsResult =
        await this.nftAdminService.syncCollectionsFromBlockchain();
