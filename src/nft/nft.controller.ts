...
export class NftController {
  ...
  constructor(private readonly nftService: NftService) {
    ...
    console.log(
      `✅ Admin wallet loaded: ${this.adminKeypair.publicKey.toString()}`,
    );
  }
  ...
  @ApiOperation({
    ...
    description:
      "Marketplace bilgileri, tüm koleksiyonlar ve item tiplerini blockchain'den çeker",
  })
  @ApiResponse({
    status: 200,
  ...
  async getCollections() {
    try {
      const { collections, itemTypesByCollection } =
        await this.nftService.fetchCollections();
      return {
        success: true,
        data: {
          collections,
  ...
  @ApiOperation({
    ...
    description:
      "DAS API ile target collection'daki tüm NFT'leri çeker (VYBE_SUPERHEROES)",
  })
  @ApiQuery({
    name: 'collection',
    description:
      'Collection mint address (optional, defaults to VYBE_SUPERHEROES)',
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          count: nfts.length,
          collection:
            collectionAddress || 'DoJfRjtn4SXnAafzvSUGEjaokSLBLnzmNWzzRzayF4cN',
        },
      };
    } catch (error) {
  ...
  @ApiOperation({
    ...
    description:
      "Belirtilen wallet adresine ait stake edilmiş NFT'leri blockchain'den çeker",
  })
  @ApiResponse({
    status: 200,
  ...
  async getPendingRewards(
    ...
    try {
      const rewards = await this.nftService.calculatePendingRewards(
        walletAddress,
        nftMint,
      );
      return {
    ...
    schema: {
      ...
      properties: {
        ...
        blockhash: {
          type: 'string',
          description:
            'Transaction blockhash (optional, will be refreshed if expired)',
        },
      },
  ...
  async mintNft(
    ...
    try {
      ...
      for (const instruction of transaction.instructions) {
        if (instruction.keys) {
          const adminMeta = instruction.keys.find(
            (meta) =>
              meta.pubkey.toString() === adminPubkeyStr && meta.isSigner,
          );
      ...
      if (!needsAdminSignature) {
        console.warn(
          `⚠️  Admin wallet ${adminPubkeyStr} may not need to sign, but signing anyway...`,
        );
      }
