...
export class TaskValidatorService {
  ...
  validateTaskConfig(
    ...
    switch (type) {
      ...
      case TaskType.SUBMIT_IMAGE:
        ...
        if (
          config?.description_required &&
          config?.description_min_length &&
          config.description_min_length < 1
        ) {
          errors.push(
            'Description minimum length must be at least 1 character',
          );
        }
        break;
  ...
  validateSubmission(
    ...
    switch (type) {
      ...
      case TaskType.SUBMIT_TEXT:
        const textConfig = config as SubmitTextConfig;
        ...
        if (!textContent) {
          ...
          if (textConfig.min_length && textContent.length < textConfig.min_length) {
            errors.push(
              `Text must be at least ${textConfig.min_length} characters`,
            );
          }
          if (textConfig.max_length && textContent.length > textConfig.max_length) {
            errors.push(
              `Text must not exceed ${textConfig.max_length} characters`,
            );
          }

          // Validate keywords
          if (textConfig.required_keywords && textConfig.required_keywords.length > 0) {
            const keywordValidation = this.validateKeywords(
              textContent,
              textConfig.required_keywords,
            );
            if (!keywordValidation.valid) {
              errors.push(
                `Missing required keywords: ${keywordValidation.missing.join(', ')}`,
              );
            }
          }
          ...
          if (textConfig.banned_words && textConfig.banned_words.length > 0) {
            ...
            const foundBanned = textConfig.banned_words.filter((word) =>
              textLower.includes(word.toLowerCase()),
            );
            if (foundBanned.length > 0) {
              errors.push(
                `Text contains banned words: ${foundBanned.join(', ')}`,
              );
            }
          }
        }
      ...
      case TaskType.SUBMIT_IMAGE:
        const imageConfig = config as SubmitImageConfig;
        ...
        if (imageConfig.description_required) {
          const description =
            submissionData.description || submissionData.image_description;
          if (!description) {
            errors.push('Image description is required');
          } else if (
            imageConfig.description_min_length &&
            description.length < imageConfig.description_min_length
          ) {
            errors.push(
              `Description must be at least ${imageConfig.description_min_length} characters`,
            );
          }
        }
        ...
        if (submissionData.metadata) {
          ...
          if (
            imageConfig.max_file_size_mb &&
            metadata.file_size_mb &&
            metadata.file_size_mb > imageConfig.max_file_size_mb
          ) {
            errors.push(
              `Image file size exceeds maximum of ${imageConfig.max_file_size_mb} MB`,
            );
          }

          // Format validation
          if (
            imageConfig.allowed_formats &&
            metadata.format &&
            !imageConfig.allowed_formats.includes(metadata.format.toLowerCase())
          ) {
            errors.push(
              `Image format must be one of: ${imageConfig.allowed_formats.join(', ')}`,
            );
          }
